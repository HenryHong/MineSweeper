...
function Board(containerId, rowsCount, colsCount) {
    ...
    for (var i = 0; i < rowsCount; i++) {
		...
		for (var j = 0; j < colsCount; j++) {
			html += "<td id='" + cellId(i, j) + "' onclick='BoxClick("+i+","+j+")' >"+DivText+"</td>";
        }
        html += "</tr>"
	}
    html += "</table></div>";
...
function BoxClick(i,j){
	...
	if ((death == 0) && (win == 0)){
		...
		if((flagswitch == 1) && (document.getElementById(cellId(i,j)).innerHTML != "O")){
		...
		else if (flagswitch == -1){
			if (document.getElementById(cellId(i,j)).innerHTML == OtherDivText){
				document.getElementById(cellId(i,j)).innerHTML = "O";
			}
			else if (document.getElementById(cellId(i,j)).innerHTML == "O"){
